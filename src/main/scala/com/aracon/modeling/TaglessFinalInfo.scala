...
object TaglessFinalInfo {
  ...
  def program[F[_]: Applicative](gets: List[String], puts: List[(String, String)])(F: KvStore[F]): F[List[String]] =
  ...
  // We cna now create a new interpreter that uses that output to precompute values and generate a new interpeter, optimised
  val optimisedInterp =
